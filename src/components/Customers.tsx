                ...
                {customers.map((customer) => {
                    return (
                        <div key={customer.id} className="h-full">
                            <div
                                className="bg-alternative relative flex flex-col items-start gap-3 text-left rounded-lg p-5 text-white">
                                <p>{customer.comment}</p>
                                <strong className="block text-xl">{customer.name}</strong>
                                <span>{customer.title}</span>
                            </div>
                            <div
                                className="relative top-5 h-28 w-28 border-4 border-primary rounded-full before:content-[''] before:absolute before:top-[-5px] before:left-10 before:w-5 before:h-5 before:bg-primary before:origin-right before:rotate-45">
                                <Image
